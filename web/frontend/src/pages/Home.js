...
const features=['Native X2DHF input cards','HF, DFT, HFS, OED, TED, SCMC modes','Libxc-enabled functional labels','Live stdout and parsed energies','Saved user history','Admin analytics'];
...
const Home=()=>(
 <div className="relative min-h-screen overflow-hidden bg-slate-950 text-slate-100">
  ...
  <header className="relative z-10 mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
   <div className="flex items-center gap-3"><img src={logo} alt="X2DHF" className="h-11 w-11 rounded-lg border border-cyan-300/30 shadow-lg shadow-cyan-500/10"/><div><div className="text-xl font-black text-white">X2DHF</div><div className="text-xs text-cyan-100/70">Native HF and DFT SaaS</div></div></div>
  ...
  <main className="relative z-10 mx-auto grid min-h-[calc(100vh-76px)] max-w-7xl grid-cols-1 items-center gap-6 px-4 pb-8 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-8">
   <section className="hero-console rounded-lg border border-cyan-300/10 bg-slate-950/60 p-6 shadow-2xl shadow-cyan-950/30 backdrop-blur">
    <div className="font-mono text-xs uppercase tracking-[.35em] text-cyan-300/75">Native quantum runtime</div>
    <h1 className="mt-3 max-w-4xl text-4xl font-black leading-tight text-white sm:text-5xl lg:text-6xl">Production workspace for web-based X2DHF.</h1>
    <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">Build native X2DHF inputs, control finite-difference grids and SCF iterations, run the original Fortran/C solver from the browser, and keep outputs tied to user/admin analytics.</p>
