...
import {useDashboardConfig} from '../config/dashboardConfig';
...
const Dashboard=()=>{
 ...
 const theme=dashboardConfig.themeValues;
 ...
 return <div className="space-y-6 text-slate-950">
  <section className={`rounded-lg border ${theme.border} ${theme.surface} p-7 shadow-sm`}>
   <div className="flex flex-wrap items-center justify-between gap-4">
    <div>
     <div className="mb-2 font-mono text-xs uppercase tracking-[.32em]" style={{color:theme.accent}}>X2DHF Command Deck</div>
     <h1 className={`text-4xl font-black tracking-tight ${theme.text}`}>{dashboardConfig.title}</h1>
     <p className={`mt-2 max-w-2xl ${theme.muted}`}>{dashboardConfig.subtitle}</p>
    </div>
    <div className="flex flex-wrap items-center gap-3">
     <div className={`rounded-full border ${theme.border} px-4 py-2 text-sm ${theme.muted}`}>{Math.round(analytics?.cpu_time_seconds||0)} CPU seconds used</div>
